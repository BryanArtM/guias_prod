...
pub async fn crear_especie(db: &Database, especie: &Especie) -> Result<i64, String> {
    ...
    conn.execute(
        "INSERT INTO especies (nombre, peso_unidad_defecto, abreviatura_trazabilidad)
         VALUES (?1, ?2, ?3)",
        [
...
pub async fn obtener_especies(db: &Database) -> Result<Vec<Especie>, String> {
    ...
    let mut result = conn.query(
        "SELECT id, nombre, peso_unidad_defecto, abreviatura_trazabilidad
    ...
    while let Some(row) = result.next().await.map_err(|e| e.to_string())? {
        especies.push(Especie {
            ...
            peso_unidad_defecto: get_optional_f64(&row, 2).map_err(|e| e.to_string())?,
            abreviatura_trazabilidad: row.get(3).map_err(|e| e.to_string())?,
        });
    }
    Ok(especies)
}
...
pub async fn actualizar_especie(db: &Database, id: i64, especie: &Especie) -> Result<(), String> {
    ...
    let conn = db.connect().map_err(|e| e.to_string())?;
    conn.execute(
        "UPDATE especies SET nombre = ?1, peso_unidad_defecto = ?2,
                abreviatura_trazabilidad = ?3
         WHERE id = ?4",
        [
            especie.nombre.clone().into(),
            option_f64_to_value(especie.peso_unidad_defecto),
